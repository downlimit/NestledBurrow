...
assert(modeSource.includes("function onBlur() {\n    if (destroyed) return;"), "late blur events cannot mutate a destroyed mode runtime");
...
assert(hudSource.includes("isInventoryInteractionBlocked: () => !inventoryModeHud.getState().peaceful.inputEnabled"), "GameHud exposes the canonical interaction block from peaceful input state");
...
console.log("Task #051 checks passed: tap/hold state, interaction blocking, loadout drag, readable labels, ear fade and presentation ownership");
