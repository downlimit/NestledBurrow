...
assert.equal(computeNeedRates({ facility: "sink", needs: DEFAULT_NEEDS }, tuning).lustre, 300);
...
assert.equal(new Set(FACILITIES.map((item) => item.id)).size, 9, "facilities have stable unique IDs");
assert.deepEqual(FACILITIES.map((item) => item.facilityType), ["shower", "toilet", "sink", "table", "cutting-table", "gas-stove", "serving-table", "lemon-sack", "juicer"]);
