...
| Дикий Атолл, стартовые арены, развилка Лес/Шахты, локальные ресурсы и будущая глубина | `systems/wild-atoll.md` | `src/world/atollWorldLayout.js`, `src/world/wildAtollDomain.js`, `src/world/wildAtollRuntime.js`; `WildAtollPrototype/` остаётся standalone design aid | `check:task-059`, `check:task-068`, `check:docs` |
