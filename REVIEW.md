...
5. устраняет blockers: механические исправляет сам, содержательные передаёт Codex через существующий PR;
...
3. начать comment с marker `<!-- integrator-codex-repair:v1 -->`, затем упомянуть `@codex` и потребовать обновить существующую PR-ветку;
4. не создавать новый branch, PR, issue или task file;
5. после Codex repair проверить delta от сохранённого reviewed head, исходные blockers, применимые checks и новый final-head CI;
6. вернуть PR в ready только после устранения известных deterministic defects.
...
@codex Implement the requested repair in this existing PR branch. Do not create a new branch or pull request.
...
Если GitHub-вызов `@codex` недоступен или не создал задачу, Интегратор выдаёт пользователю одну готовую строку для Codex без переноса технического текста:
...
Пользователь только отправляет эту строку в Codex. Он не копирует список defects, не открывает новый Lead-запрос и не управляет branch/SHA/checks.
...
- При передаче содержательного repair сообщить либо что `@codex` запущен и действий пользователя не требуется, либо дать одну готовую fallback-строку для вставки в Codex.
