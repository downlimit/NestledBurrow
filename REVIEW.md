...
Пользователь отвечает за вижн и оценку игры, но не обслуживает PR, ветки, CI, connector routing или Markdown. Если текущий Integrator-чат не имеет прямого инструмента запуска Codex, пользователь выполняет один минимальный launch-шаг: отправляет в Codex короткую команду `почини PR <номер>`.
...
6. если прямой launch недоступен — дать пользователю короткую команду `почини PR <номер>`;
