...
test("Alt tap toggles peaceful/combat and hidden inventory shortcuts stay inactive", async ({ page }, testInfo) => {
  test.skip(testInfo.project.name.startsWith("mobile"), "Task #051 defines desktop physical Alt only");
  await boot(page);
...
test("new game restarts the scene without touching destroyed inventory zones", async ({ page }, testInfo) => {
  test.skip(testInfo.project.name.startsWith("mobile"), "Task #051 lifecycle regression is covered on desktop");
  const pageErrors = [];
  page.on("pageerror", (error) => pageErrors.push(error.message));
  await boot(page);
  await page.evaluate(() => {
    window.__NESTLED_BURROW_E2E__.__task051PreviousBridge = true;
  });
  ...
  await expect.poll(() => page.evaluate(
    () => Boolean(window.__NESTLED_BURROW_E2E__)
      && !window.__NESTLED_BURROW_E2E__.__task051PreviousBridge,
