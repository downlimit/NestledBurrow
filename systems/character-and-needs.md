...
- Every resident resolves to a stable male/female demographic sex; generated newborns use a deterministic distribution that stays near `50/50`. A `partner`/marriage pair is valid only between opposite sexes; invalid same-sex partner edges are rejected/repaired rather than becoming families. Husband/wife surname roles follow the actual pair sexes instead of an arbitrary side assignment.
...
- `youngAdult`/`adult` couples may have `1..3` children ≥`6` days apart. Mean births/day: `6@<=240`, `5@260`, `4@280`, `3@300`, `2@320`, `1@340`, `0@>=360`, interpolated with daily variation. Shared ancestry through great-grandparents forbids pairing; farther known ancestry/shared surname only lower pairing priority. Dense bloodlines progressively lose birth priority and may be capped at fewer children. Visible surname diversity is softly regulated toward ~`90`: intervention starts before collapse at `105` living unique surnames and strengthens below `90`, with `75` as the strong-pressure floor. Some normal replenishment slots then become adult arrivals, at most `2` per day; when possible `95%` of them restore an extinct surname root and `5%` may introduce a previously unused surname. Arrivals do not add invented persistent relatives; missing ancestry remains display-only.
- Children persist and inherit spending, food preferences and visit periods with variation. Skills/talents are not implemented. No general time acceleration; sleep advances world time. `populationDomain` owns person data; `populationLifecycleDomain` owns generations; `personNames`, `personFamilyNames` and `populationLineageBalance` own names, surname policy and long-run lineage renewal.
...
- Hover shows NESTLD after `667 ms`; at `1334 ms` the card expands into a family tree. Missing ancestors are deterministic display-only placeholders, but fictional couples are generated as coherent opposite-sex family lines: spouse surnames follow the same visual marriage logic and grandparent branches preserve plausible birth-family surnames instead of assigning every ancestor an unrelated surname. Every full name is hard-clipped to its own cell. Hovering a long name starts its marquee immediately; if the cursor remains, the completed loop returns to the start, waits `1 s`, then repeats. Cursor leave lets the active loop finish and then stops. `СЕМЬЯ/FAMILY` is centered over the tree; fully expanded header adds the localized age stage with sex-aware Russian grammar. Coarse pointers use long-press and bar edits persist.
...
`populationDomain` owns person data and the named baseline; `populationLifecycleDomain` expands worlds to 300 and advances generations. `personNames`/`personFamilyNames`, `populationLineageBalance`, `guestIntentDomain`, `personInspectionRuntime` and `personFamilyTree` own names, long-run lineage renewal, live lifecycle, inspection and missing-history presentation.
