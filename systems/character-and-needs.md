...
- Generated residents use a deterministic `1000`-name pool; legacy `Resident N` names are repaired. Real residents also have stable surnames: unrelated founder lines are distinct where possible. On pairing: `89%` wife-side takes husband-side surname, `5%` both keep, `5%` reverse, `1%` both take correctly hyphenated `A-B`; children inherit paternal surname `90%`, maternal `10%`.
