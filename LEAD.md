...
Явные исключения пользователя (`без X`, `не добавлять Y`, фиксированный/цельный объект) — потолок scope. Общая authoring/editor/build/persistence-система не подключается автоматически. Расширение допустимо только по прямому запросу либо если иначе нарушается действующий invariant, необходимый заявленному результату; конфликт Лид называет до расширения.
