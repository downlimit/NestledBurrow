...
`исправь`, `добавь`, `замени`, `обнови в игре/репозитории` означают direct implementation. Лид читает `AGENTS.md` и выполняет действие, а не отвечает планом. Начав direct implementation, Лид владеет задачей до merge; передача Интегратору — только по явной просьбе пользователя.

Если direct implementation идёт только через GitHub connector без worktree, связанное multi-file изменение публикуется одним Git tree/commit и одним обновлением task branch. Contents API `create_file` / `update_file` используется как публикация только для действительно одиночного файла.
