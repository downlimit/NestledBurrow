...
const BUILD_KINDS = new Set(["wall", "wall-node", "ground", "floor", "carpet", "tree", "placed"]);
...
export function captureStartingLayout(scene) {
  ...
  const buildObjects = [...scene.buildPlacedObjects.values()].map((object) => {
    const { sprites: _sprites, ...serializable } = object;
...
export function loadStartingLayout(storage = globalThis.localStorage) {
  const source = storage?.getItem(STARTING_LAYOUT_STORAGE_KEY);
  if (!source) return null;
  return normalizeStartingLayout(JSON.parse(source));
}
...
function restoreBeds(scene, definitions) {
  ...
  for (const definition of definitions) {
    if (definition.id.startsWith("editor-bed-")) {
      runtime.addBed({ x: definition.position.x - TILE_SIZE / 2, y: definition.position.y - TILE_SIZE / 2 });
    } else {
      runtime.restoreBed(definition);
    }
  }
}
...
export function findBuildTreeColliderSelection(scene, point) {
  if (!scene?.buildPlacedObjects || !scene?.worldLayout?.getWorldObjectColliders) return null;
  const tree = [...scene.buildPlacedObjects.values()]
    .reverse()
    .find((object) => object.kind === "tree" && pointInside(point, object.bounds));
  if (!tree) return null;
  return scene.worldLayout.getWorldObjectColliders().find((entry) => entry.id === tree.id) ?? null;
}
