import { BED_OBJECT, DEBRIS_OBJECTS } from "./debrisConfig.js";
...
export function createDebrisRuntime(scene, { sessionState, worldLayout }) {
  ...
  function stateFor(definition) { return sessionState.gameplay.debris[definition.id]; }
  ...
  function createVisual(definition) {
    ...
    drawLog(graphics, stateFor(definition)?.remainingHits ?? 5);
  ...
  function hitWithFeedback(debrisId, result, onComplete = () => {}) {
    const definition = DEBRIS_OBJECTS.find((item) => item.id === debrisId);
  ...
  return {
    getInteractionDefinitions() { return [...DEBRIS_OBJECTS.filter(isPresent), BED_OBJECT]; },
    isPresent(id) { const definition = DEBRIS_OBJECTS.find((item) => item.id === (id ?? DEBRIS_OBJECTS[0].id)); return definition ? isPresent(definition) : false; },
    hitWithFeedback,
    clearWithFeedback,
    setSleeping,
    destroy() { destroyed = true; for (const graphics of visuals.values()) graphics.destroy(); visuals.clear(); bedGraphics?.destroy(); sleepGraphics?.destroy(); worldLayout.blocked.delete(key(BED_OBJECT)); for (const d of DEBRIS_OBJECTS) worldLayout.blocked.delete(key(d)); },
  };
}
